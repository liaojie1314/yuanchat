...
| xs   | 2    | `rounded-sm`    | 气泡尾角（与 8px 主体形成"尖角"）                 |
| s    | 4    | `rounded-[4px]` | 角标、进度端点、代码内联                          |
| m    | 8    | `rounded-lg`    | **上限档**：按钮、输入框、卡片、气泡、Modal、面板 |
| full | 9999 | `rounded-full`  | 头像、圆形按钮、徽章、状态点、滑块轨道            |

**硬约束**：组件圆角一律 **≤ 8px（`rounded-lg`）**，禁止 `rounded-xl` / `rounded-2xl` /
`rounded-3xl` / `rounded-[28px]`；只有"本身就是圆形"的元素用 `rounded-full`。
本文件后续各组件小节里仍写着 "圆角 12px / 16px" 的地方以本表为准（那些描述早于该约束，
代码已统一到 8px），改动组件时不要照抄。

**圆角语义**：8px 是全局统一的"柔和但克制"档位——不靠圆角大小区分层级，
层级交给背景色（surface-container-\*）与 elevation 表达。
