...
>
> **执行状态（2026-08-23，代码审计核实，基线 `bc4ae8d`）**：v1.0 路线图 **23 个批次
> 中 21 个已完成**（此前记作「21 中 19」，批次总数与完成数均为笔误）。v1.0 路线图之外
> 又追加完成了 A6 / A7 / H1 与「对象级读授权 + 离线对象 GC」四项。
>
...
> 该 outline 的 A6 / A7 / H1 已完成，剩余 F0-F2（通话）、G1-G2（朋友圈）、H1b（表情商城）。
> **当前在做**：A8（auth 补全与安全加固）→ H1b（表情商城）→ B6（文档与进度真源对齐）。
>
...
| 当前       | auth 补全与安全加固 | **A8**                     | 1-1.5 周 | 🔨 进行中                 |
| 紧随       | 表情商城 + 文档真源 | H1b + B6                   | 1.5-2 周 | 计划就绪                  |
...
| H1b  | 表情商城 + 自主发布     | 计划就绪  | 迁移 **015**；设计文档已按 2026-08-23 漂移审计修正       |
...
### 新批次依赖（A8 / B6-B9 / C5-C9 / J1-J5 / F / G / H1b）

```
A8 ─────────────────────────  auth 补全（无依赖，当前在做）
     ▲
     └── B8 前置更省事（OTP/QR 测试直接吃到 miniredis + testutil）

B6 → B7 → B9                  文档真源 → 门禁 → 契约收敛
B8                            独立（建议提到 A8 之前）
...
H1b                           依赖 H1（已完成）；**必须先补 GC 引用登记**
F0 → F1 → F2                  spike 先行
G1 → G2                       后端先行
```

> H1b 的 GC 前置不是可选项：`sticker_packs.cover_url` 未登记进
> `ReferencedKeys`，不补就等于给封面对象埋了个 7 天定时删除。
> 见 [MASTER_PLAN 未做清单总览](MASTER_PLAN.md#未做清单总览单一真源)「既有代码的真实缺陷」。
