...
> 该 outline 的 A6 / A7 / H1 / H1b / F0-F2（通话）均已完成，剩余 G1-G2（朋友圈）。
...
| （未发版） | 实时通话            | **F0 + F1 + F2**           | 3-4 周   | ✅ 已在 dev（2026-09-12） |
...
| 批次 | 主题                    | 状态      | 备注                                                                    |
| ---- | ----------------------- | --------- | ----------------------------------------------------------------------- |
| F0   | WebRTC 跨端可行性 spike | ✅ 已完成 | 结论：Linux WebKitGTK 无 GstWebRTC，改走原生助手进程（设计文档 §3.11a） |
| F1   | 1v1 语音通话            | ✅ 已完成 | coturn `:3478`，HMAC 临时凭据                                           |
| F2   | 视频通话                | ✅ 已完成 | 含群通话 mesh（上限 4 人）与 Linux 原生视频后端                         |
