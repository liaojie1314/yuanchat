...
| 级别 | 条目                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🔴   | **生产 TURN 完全没接线**：`docker-compose.prod.yml` 部署了 coturn，但 `server_env` 锚点里没有任何 `YUANCHAT_TURN_*`，服务端密钥取到空值 → `ice-servers` 只返回 STUN，且 `turn.host` 退回 `localhost`；同时 `coturn/turnserver.prod.conf` 的 `__TURN_SECRET__`/`__DOMAIN__`/`__PUBLIC_IP__` 三个占位符 `deploy/install.sh` 并不替换。结论：**生产环境跨 NAT 的通话连不上**。dev 链路不受影响（dev 的 compose 与配置都是全的） |
