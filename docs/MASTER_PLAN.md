...
| B      | 运营仪表盘：`GET /admin/stats` 聚合指标 + 概览页、好友申请/OTP 量、Web Push 订阅视图                                                                  | 进行中 |
| C      | 贴纸包治理：admin 全量管理页、untakedown + is_official 端点（写审计）、flagged 包商城暂隐（发布者/已添加者保留，清标记恢复）                          | ✅     |
| D      | 补漏小项：admin 重置头像（P0-3）、存储统计视图（P1-3）、admin 删消息级联清收藏（P2-1）                                                                | 待派   |
| 搭车   | captcha 两缺陷修复、DB_SCHEMA 按 001-015 重建（compose 镜像核实已钉版本）                                                                             | ✅     |
| 债收口 | `/auth/*` 前缀统一、WS 贴纸发送口径收紧（未下架+已添加/官方+未打标）、`GET /sticker-packs` 分页、限流改 Redis、Dispatcher 分布式、B7 覆盖率门禁       | 进行中 |
